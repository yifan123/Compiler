...
#define	MULTI_ID_DEF		2		//重定义标识符
#define FUNC_RPARA_TYPE_ERROR 3      //函数调用时，传进的实参类型不匹配
#define FUNC_RPARA_NUM_ERROR  4      ////函数调用时，传进的实参个数不匹配
#define VAR_TYPE_ERROR		5		//变量类型错误，变量只允许是int或者char类型
#define OPRAND_TYPE_ERROR   6        //运算数类型只允许是int或者char类型
#define ARRAY_INDEX_TYPE_ERROR   7 //数组下标类型错误，必须为整型
#define	FUNC_NO_RET			33		//无返回值函数不能在表达式中使用
